...
class Component {
  ...
  validate (done) {
    ...
    this.directives.forEach((directive) => {
      if (directive.cmd === 'EXPOSE') this.params.port = directive.params.port
      if (directive.cmd === 'LIFECYCLE') {
        if (!this.params.lifecycles) this.params.lifecycles = []
        this.params.lifecycles.push(directive.params.lifecycle)
      }
      if (directive.cmd === 'CRON') this.params.cron = directive.params
      if (directive.cmd === 'CPU') this.params.cpu = parseFloat(directive.params.units)
      if (directive.cmd === 'MEMORY') this.params.memory = parseInt(directive.params.units)
      if (directive.cmd === 'STATIC') this.params.contentDirectory = directive.params.contentDirectory
