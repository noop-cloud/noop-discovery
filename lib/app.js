...
class Application extends EventEmitter {
  ...
  setupWatch (done) {
    async.auto({
      ...
      watchLocations: ['findWatchLocations', (results, done) => {
        results.findWatchLocations.forEach((dir) => {
          this.setDir(dir)
    ...
    this.watcher.on('change', (file, stats) => {
      ...
      for (const manfestPath in this.watching) {
        const rootPath = path.parse(manfestPath).dir
        if (file.startsWith(rootPath) && file !== rootPath) {
          for (const component in this.watching[manfestPath]) {
            if (this.watching[manfestPath][component].ig.ignores(file.slice(rootPath.length + 1))) {
      ...
      const removeSubWatchers = (path) => {
        this.directories[path].forEach((base) => {
          this.watcher.remove(`${path}/${base}`)
          if (`${path}/${base}` in this.directories) removeSubWatchers(`${path}/${base}`)
        })
        delete this.directories[path]
      }

      if (file in this.directories) {
        fs.readdir(file, (err, files) => {
          if (!err) {
            let emitStatus = false
            files.forEach((base) => {
              if (this.directories[file].has(base)) {
                this.directories[file].delete(base)
              } else {
                emitStatus = true
                this.watcher.add(`${file}/${base}`)
                fs.stat(`${file}/${base}`, (err, stats) => {
                  if (!err) {
                    if (stats.isDirectory()) {
                      this.setDir(`${file}/${base}`)
                    }
                  }
                })
              }
            })
            this.directories[file].forEach((base) => {
              this.watcher.remove(`${file}/${base}`)
              if (`${file}/${base}` in this.directories) removeSubWatchers(`${file}/${base}`)
              emitStatus = true
            })
            this.directories[file] = new Set(files)
            if (emitStatus) emit()
          }
        })
      }
  ...
  setDir (dir) {
    fs.readdir(dir, (err, files) => {
      if (!err) this.directories[dir] = new Set(files)
    })
  }

  checkFile (file, done) {
    if (path.parse(file).base === 'Noopfile') return true
