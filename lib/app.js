...
class Application extends EventEmitter {
  ...
  discover (done) {
    async.auto({
      findFiles: (done) => {
        const recursive = (dir, files = []) => {
          fs.readdirSync(dir).forEach((file) => {
            if (fs.statSync(dir + '/' + file).isDirectory()) {
              if (!(file === '.git' && dir === this.rootPath)) {
                files = recursive(dir + '/' + file, files)
              }
            } else if (file === 'Noopfile' || file === '.gitignore' || file === '.dockerignore') {
              files.push(path.join(dir, '/', file))
            }
          })
          return files
        }
        done(null, recursive(this.rootPath))
      },
